...
本系统采用前后端分离架构，基于SpringBoot+Vue技术栈实现。系统分为博客网站、管理后台两个部分，主要功能模块包括用户管理、文章管理、评论管理、分类标签管理、说说管理、资源管理和系统配置。

系统架构设计遵循分层原则，后端采用Controller-Service-Mapper三层架构，前端采用Vue组件化开发。数据存储使用MySQL数据库，通过MyBatis-Plus进行ORM操作。认证授权使用Sa-Token框架实现，确保系统安全性。
