...
> 用例图已生成PlantUML代码文件：[unlogged_user_usecase.puml](file:///workspace/unlogged_user_usecase.puml)
> 
> 您可以使用以下方式之一查看专业的UML用例图：
> 1. 访问 http://www.plantuml.com/plantuml/uml/ ，将puml文件内容粘贴进去
> 2. 使用VSCode的PlantUML插件
> 3. 下载PlantUML jar文件在本地运行
...
rectangle "个人博客系统" {
    ...
    usecase "查看分类" as ViewCategories
    usecase "查看标签" as ViewTags
    usecase "查看说说" as ViewMoments
    usecase "查看评论" as ViewComments
    usecase "搜索文章" as SearchArticles
    usecase "登录账号" as Login
    usecase "注册账号" as Register
}

UnloggedUser --> BrowseHome
UnloggedUser --> ViewArticleList
UnloggedUser --> ViewArticleDetail
UnloggedUser --> ViewCategories
UnloggedUser --> ViewTags
UnloggedUser --> ViewMoments
UnloggedUser --> ViewComments
UnloggedUser --> SearchArticles
UnloggedUser --> Login
...
> 用例图已生成PlantUML代码文件：[logged_user_usecase.puml](file:///workspace/logged_user_usecase.puml)
...
rectangle "个人博客系统" {
    ...
    usecase "发表评论" as PostComment
    usecase "回复评论" as ReplyComment
    usecase "点赞评论" as LikeComment
    usecase "发布说说" as PublishMoment
    usecase "编辑说说" as EditMoment
    usecase "删除说说" as DeleteMoment
    usecase "点赞说说" as LikeMoment
    usecase "上传图片" as UploadImage
    usecase "上传文件" as UploadFile
    usecase "管理个人信息" as ManageProfile
    usecase "修改密码" as ChangePassword
    usecase "退出登录" as Logout
}

LoggedUser --> BrowseHome
LoggedUser --> ViewArticleList
LoggedUser --> ViewArticleDetail
LoggedUser --> ViewCategories
LoggedUser --> ViewTags
LoggedUser --> ViewMoments
LoggedUser --> ViewComments
LoggedUser --> SearchArticles

LoggedUser --> PublishArticle
LoggedUser --> EditArticle
LoggedUser --> DeleteArticle
LoggedUser --> PostComment
LoggedUser --> ReplyComment
LoggedUser --> LikeComment
LoggedUser --> PublishMoment
LoggedUser --> EditMoment
LoggedUser --> DeleteMoment
LoggedUser --> LikeMoment
LoggedUser --> UploadImage
LoggedUser --> UploadFile
LoggedUser --> ManageProfile
...
> 用例图已生成PlantUML代码文件：[admin_usecase.puml](file:///workspace/admin_usecase.puml)
...
rectangle "个人博客系统" {
    ...
    usecase "文章审核" as ArticleApproval
    usecase "评论审核" as CommentApproval
    usecase "分类管理" as CategoryManagement
    usecase "标签管理" as TagManagement
    usecase "资源管理" as ResourceManagement
    usecase "系统配置" as SystemConfig
    usecase "查看系统日志" as ViewSystemLogs
    usecase "管理缓存" as ManageCache
}

Admin --> BrowseHome
Admin --> ViewArticleList
Admin --> ViewArticleDetail
...
Admin --> ArticleApproval
Admin --> CommentApproval
Admin --> CategoryManagement
Admin --> TagManagement
Admin --> ResourceManagement
Admin --> SystemConfig
